...
GENRES = ('classic pop and rock', 'punk', 'pop', 'dance and electronica', 'jazz and blues')
    ...
    for genre in GENRES:
        ...
        for artist in artists:
            ...
            for path in map(lambda x: path_from_trackid(msddir, x), track_ids):
                ...
                feats_name_list = list(feats_name)
                counter = len(feats_list)
                counter2 = len(feats_name_list)
                assert counter == counter2, 'feat length problem, len(feats)=' + str(counter)
                output.write(genre)
                for k in range(counter):
                    output.write(',')
                    output.write(feats_list[k])
                output.write('\n')
